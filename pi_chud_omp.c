...
// Saves exactly N decimal digits to "pi.txt" and prints a short preview + timing.
//
// Build on Intel macOS (Homebrew clang + GMP + libomp):
//   /usr/local/opt/llvm/bin/clang -std=c11 -O3 -march=native pi_chud_omp.c -o pi_chud_omp \
...
//
// Run (use all cores):
//   OMP_NUM_THREADS=4 OMP_DYNAMIC=FALSE OMP_PROC_BIND=TRUE ./pi_chud_omp 100000
...
static void bs(long a, long b, PQT* out){
    ...
    // Q = a^3 * C3
        ...
        mpz_mul(out->Q, out->Q, out->Q);         // a^3
        mpz_mul(out->Q, out->Q, C3);
    // T = P * (13591409 + 545140134 a); sign (-1)^a
...
// Format mpf π to fixed decimal with exactly `digits` digits and save to file.
static int write_pi_to_file(mpf_t pi, unsigned long digits, const char* filename){
    long exp10 = 0;
    char* s = mpf_get_str(NULL, &exp10, 10, digits + 1, pi); // (digits+1) significant digits
    if (!s) return -1;
...
int main(int argc, char** argv){
    unsigned long digits = (argc > 1) ? strtoul(argv[1], NULL, 10) : 100000ul;
    ...
    for (int i = 0; i < T; ++i) {
        ...
        if (a < b) bs(a, b, &part[i]);
    }

    // Reduce (combine) sequentially
